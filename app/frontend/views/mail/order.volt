<p>Добрый день! На сайте была оставлена новая заявка на товар с такими данными:</p>
<table border="1" style="width: 600px;">
    <tr>
        {% if name %}<th>Имя</th>{% endif %}
        <th>Телефон</th>
        {% if email %}<th>Email</th>{% endif %}
        <th>Дата заказа</th>
    </tr>
    <tr>
        {% if name %}<td><strong>{{ name }}</strong></td>{% endif %}
        <td><strong>{{ phone }}</strong></td>
        {% if email %}<td><strong>{{ email }}</strong></td>{% endif %}
        <td><strong>{{ datetime }}</strong></td>
    </tr>
</table>
<br>
<br>
<table id="cart_summary" class="table table-bordered stock-management-on"  style="width: 600px;">
    <thead>
    <tr>
