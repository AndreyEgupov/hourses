...
$(document).ready(
	function ()
	{
		...
		$('#pattern-box').find('li').click(
			function()
			{
				if($('#layout option:selected').val() != 'wide'){
					location.href = location.href.replace(/&pattern=[^&]*/, '')+'&pattern='+$(this).attr('class');
				}
			}
		);
		...
		$('#reset').click(
			function()
			{
				location.href = location.href.replace(/&theme=[^&]*/, '&theme=theme1').replace(/&theme_font=[^&]*/, '&theme_font=font1').replace(/&theme_layout=[^&]*/, '&theme_layout=wide');
			}
		);
		...
		$('#layout').change(
			function()
			{
				location.href = location.href.replace(/&theme_layout=[^&]*/, '')+'&theme_layout='+$('#layout option:selected').val();
			}
		);

		$('#gear-right').css({'left' : '215px'});
		...
		if($('#layout option:selected').val() == 'boxed'){
			
		
			$_partern = $('#pattern-box li').filter('.active');
			$_cls_partern = $_partern.attr('class');
			$_cls_partern = $_cls_partern.replace('active', '');
			$('body').addClass($_cls_partern);
			
		}
		else{
			
			$_partern = $('#pattern-box li');
			$_partern.each(function($i){
				$(this).click(function (e) {
					 alert ('Select For Layout Style: Boxed ') ;
				});
			});
